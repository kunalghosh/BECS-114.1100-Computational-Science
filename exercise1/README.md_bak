Solution to Question 1 {#prob1}
======================

In this question we have been asked to compute the approximate values of
$\epsilon_t$ the truncation error and $\epsilon_r$ the round off error
of the function $f(x) = sin(x)$ at $x=0.5$

The round off error $(\epsilon_r)$ is caused by the limited numerical
precision of computers. i.e. if our computer is correct upto 6
significant digits then 5.000000 will be equal to 5.0000001 however
these two numbers are not equal. Therefore there is clearly, a
possibility of error in representing numbers with fixed precision.

The truncation error $(\epsilon_t)$ is caused when we try to use
infinite series expansion (for example taylor series) in our computation
and we limit the number of terms in the expansion. For the terms that
are not included in the expansion, an error is introduced. for example
the truncation error using Taylor’s theorem is $-\frac{h}{6}f'''(\zeta)$

The total error $\epsilon$ is calculated by summing up truncation and
round off errors. $$\epsilon = \epsilon_t + \epsilon_r$$

Specifically, we have been asked to analyse the $sin(x)$ function.
assuming $\zeta = 0.5$ we get the truncation error as:
$$\epsilon_t = -\frac{h}{6}(-cos(0.5))
\label{eqn:et}$$

Also, the approximate value of the first derivative of a function using
central limit formula is: $$f'(x) \approx \frac{f(x+h) - f(x-h)}{2h}$$

Since we need to calculate the first derivative about $ x = 0.5 $ then
the above equation becomes..

$$f'(x) \approx \frac{f(0.5+h) - f(0.5-h)}{2h}$$

and the analytical first derivative of $sin(x)$, the function under
consideration, is $cos(x)$. So the total error $\epsilon$is calculated
as..

$$\epsilon \approx \abs{\frac{f(0.5+h) - f(0.5-h)}{2h} - cos(x)}
\label{eqn:fdash}$$

and from \[eqn:fdash\] and \[eqn:et\] we get..

$$\epsilon_r = \epsilon - \epsilon_t
\label{eqn:er}$$

which is also a function of $h$. Now we generate 25 (arbitrary, we could
have chosen more) values of $h$ by the following update equation
$h \leftarrow {h} / {4}$.

Plotting \[eqn:fdash\] , \[eqn:et\] and \[eqn:er\] as a function of $h$
on the log scale gives us \[fig:solution1\_fig\]. We use the log scale
because in that scale really small values are scaled up which helps us
visualize the values better.

![Total, truncation and rounding errors for $sin(x)$ at $x = 0.5$.<span
data-label="fig:solution1_fig"></span>](solution1_fig)

The corresponding python code can be found at \[code:problem1\]

Solution to Question 3 {#prob3}
======================

![The error between analytical solution and taylor approximation. First
row is for equation $(e^x - 1)/x$ and the second row for equation
$(e^x - e^{-x}) / 2x$. The third column shows the graphs in the second
column magnified at the point of inflexion.<span
data-label="fig:solution3_fig"></span>](figure_3.pdf)
  ...
  :  Table logging the Euler value approximations every 100 iterations.
  The second and the third columns correspond to the convergence
  equation without the constant and the third and fourth and fifth are
  ones with a constant (0.5 in this case) <span
  data-label="table:1"></span>

The corresponding python code can be found at \[code:problem3\]

Solution to Question 5 {#prob5}
======================

In this problem we have to calculate the error in analytical values and
approximate Taylor values of given functions arguments tend to zero. The
taylor approximates are useful because as the value of arguments tend to
zero the analytical functions would become really large since the
argument is in the denominator.

Deriving the taylor approximate of $f_1(x) = \frac{e^x - 1}{x}$ upto the
4th term (arbitrarily chosen) $$f_1(x) = \frac{1}{x} (e^x - 1)
\label{eqn:q5_1}$$ From taylor’s expansion
$$e^x = e^0 + \sum_{i=1}^{n-1} \frac{x^i}{i!} e^{(i)}(0) + \frac{x^n}{n!} e^{(n)}(\zeta)
\label{eqn:q5_2}$$ substituting \[eqn:q5\_2\] (upto the 4th term i.e.
n=4)in \[eqn:q5\_1\] and simplifying ( the $x$ in the denominator
cancels out) we get.
...
The approximate value of \[eqn:q5\_1\] is calculated using \[eqn:q5\_4\]
for small values of $x$ and then the two are subtracted to get the error
which is plotted in \[fig:solution5\_fig\]

Similarly for $f_2(x) = (e^x - e^{-x}) / 2x$ Deriving the taylor
approximate upto the 4th term we get.
...
As for the first function, approximate value of \[eqn:q5\_5\] is
calculated using \[eqn:q5\_6\] for small values of $x$ and then the two
are subtracted to get the error which is plotted in
\[fig:solution5\_fig\]

![<span data-label="fig:solution5_fig"></span>](solution_5_fig.pdf)

The corresponding python code can be found at \[code:problem5\]

Appendix A {#code:problem1}
==========

Python source code for \[prob1\].
...
Appendix B {#code:problem3}
==========

Python source code for \[prob3\].
...
Appendix C {#code:problem5}
==========

Python source code for \[prob5\].
